...
import { 
  ...
  TextInput, Modal, Animated, Easing, SafeAreaView, Image,
  KeyboardAvoidingView, Platform
} from 'react-native';
import { Plus, Trash2, Edit2, Check, Users } from 'lucide-react-native';
...
export default function WheelScreen({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  const spin = () => {
    ...
    Animated.timing(rotation, {
      ...
      const winningIndex = Math.floor(((360 - (normalizedRotation - 90)) % 360) / degreesPerSegment);
          ...
          <View style={styles.headerTop}>
            <View style={{ flex: 1 }}>
              ...
              {isEditingName ? (
                ...
                <TouchableOpacity style={styles.nameRow} onPress={() => { setTempName(activeWheel.name); setIsEditingName(true); }}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.wheelContainer}>
            ...
            {activeWheel.options.length < 2 && (
              <Text style={styles.hintText}>Add at least 2 options to spin!</Text>
            )}
          </View>
          ...
          <View style={styles.optionsSection}>
            ...
            <View style={styles.optionsList}>
              {activeWheel.options.map((opt, index) => (
                <View key={opt.id} style={[styles.optionItem, index === activeWheel.options.length - 1 && { borderBottomWidth: 0 }]}>
        ...
        <Modal visible={showResultModal} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.resultCard}>
              ...
              <TouchableOpacity 
                style={[styles.modalButton, { backgroundColor: '#f2f2f7' }]} 
                onPress={() => {
...
const getStyles = (isDarkMode: boolean) => StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingTop: 10, backgroundColor: isDarkMode ? '#1c1c1e' : '#ffffff', borderBottomWidth: 1, borderBottomColor: isDarkMode ? '#38383a' : '#c6c6c8' },
  ...
  wheelName: { fontSize: 28, fontWeight: 'bold', color: isDarkMode ? 'white' : '#1c1c1e', marginRight: 8 },
  ...
  tab: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: isDarkMode ? '#2c2c2e' : '#e5e5ea', marginRight: 8 },
  activeTab: { backgroundColor: '#007AFF' },
  tabText: { fontSize: 14, fontWeight: '600', color: isDarkMode ? '#8e8e93' : '#8e8e93' },
  activeTabText: { color: 'white' },
  addWheelTab: { padding: 8, borderRadius: 20, backgroundColor: isDarkMode ? '#2c2c2e' : '#e5e5ea', width: 36, alignItems: 'center' },
  scrollContent: { padding: 20 },
  wheelContainer: { alignItems: 'center', marginVertical: 30 },
  spinButton: { backgroundColor: '#007AFF', width: 180, height: 56, borderRadius: 28, justifyContent: 'center', alignItems: 'center', marginTop: 40, elevation: 5, shadowColor: '#007AFF', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 },
  spinButtonDisabled: { backgroundColor: isDarkMode ? '#3a3a3c' : '#c7c7cc', shadowOpacity: 0 },
  ...
  optionInput: { flex: 1, height: 50, backgroundColor: isDarkMode ? '#2c2c2e' : 'white', borderRadius: 12, paddingHorizontal: 15, color: isDarkMode ? 'white' : 'black', borderWidth: 1, borderColor: isDarkMode ? '#38383a' : '#c6c6c8' },
  ...
  optionsList: { backgroundColor: isDarkMode ? '#2c2c2e' : 'white', borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: isDarkMode ? '#38383a' : '#c6c6c8' },
  optionItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: isDarkMode ? '#38383a' : '#f2f2f7' },
  optionInfo: { flexDirection: 'row', alignItems: 'center' },
  colorDot: { width: 12, height: 12, borderRadius: 6, marginRight: 12 },
  optionLabel: { fontSize: 16, fontWeight: '500', color: isDarkMode ? '#e5e5ea' : '#1c1c1e' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  resultCard: { backgroundColor: isDarkMode ? '#2c2c2e' : 'white', width: '100%', borderRadius: 32, padding: 30, alignItems: 'center' },
  ...
  modalButton: { backgroundColor: '#007AFF', width: '100%', height: 56, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
